-- ══════════════════════════════════════════════════════
-- Seed: شجرة الحسابات الكاملة
-- قم بتشغيل هذا السكربت في قاعدة البيانات (مثل phpMyAdmin)
-- يفضل إفراغ جدول الحسابات أولاً لضمان عدم وجود تكرار
-- ══════════════════════════════════════════════════════
...
-- IsLeaf: 1 if it has no children, 0 if it is a parent
-- AllowPosting: 1 if Yes, 0 otherwise
...
('3402', 'الأرباح المبقاة', 'أرباح مبقاة لغرض إعادة استثمارها في أعمال المنشأة', 3, 2, (SELECT Id FROM (SELECT Id FROM Accounts WHERE Code='34') x), 3, 1, 0, 1, NOW()),
