...
public class TaskBlueprint : BaseEntity
{
    ...
    public InteractiveTaskType TaskType { get; set; } = InteractiveTaskType.General;
    
    public string TaskBehavior { get; set; } = "RandomInventory"; // RandomInventory, FixedChecklist, etc.
    
    // Default config for tasks generated by this blueprint
...
public enum InteractiveTaskType
{
    General = 1,                 // مهام عامة
    InventoryCount = 2,          // جرد مخزون
    PosShiftClosure = 3,         // إغلاق وردية كاشير
    OrderFulfillment = 4,        // تجهيز طلبات أونلاين
    ReceivePurchaseOrder = 5,    // استلام بضاعة
    RestockingShelves = 6,       // تعبئة وتنسيق رفوف
    BranchTransferPacking = 7,   // تجهيز نقل لفروع
    CustomerFollowUp = 8,        // متابعة عملاء
    StoreOpeningChecklist = 9,   // تجهيز ونظافة الفرع
    VisualMerchandising = 10     // تنسيق العرض البصري (مانيكان)
}
...
public class EmployeeTask : BaseEntity
{
    ...
    // e.g. OrderId, TransferId, depending on TaskType
    public int? ReferenceId { get; set; }
    
    // Optional settings for this specific task
    public string? TaskConfigJson { get; set; }
...
public class EmployeeTaskItem : BaseEntity
{
    ...
    public int? ReferenceId { get; set; } // Reference to line item, customer, etc.
