...
public class MappingSeederController : ControllerBase
{
    ...
    public async Task<IActionResult> SeedChartOfAccounts()
    {
        ...
        try
        {
            ...
            var concreteAccounts = new List<Account>
            {
                ...
                new Account { Code = "1106", NameAr = "مخزون البضائع", NameEn = "Inventory Asset", Type = AccountType.Asset, Nature = AccountNature.Debit, Level = 3, ParentId = currentAssets.Id, IsLeaf = false, AllowPosting = false, IsSystem = true, CreatedAt = TimeHelper.GetEgyptTime() },
                new Account { Code = "110601", NameAr = "مخزن بضائع لدى شركة الشحن", NameEn = "Courier Inventory", Type = AccountType.Asset, Nature = AccountNature.Debit, Level = 4, ParentId = null /* Will be handled by tree hierarchy logic */, IsLeaf = true, AllowPosting = true, IsSystem = true, CreatedAt = TimeHelper.GetEgyptTime() },
    ...
    private async Task<List<string>> AutoLinkInternal()
    {
        ...
        var searchMap = new Dictionary<string, string[]>
        {
            ...
            { MappingKeys.CourierInventory, new[] { "110601", "مخزن شركة الشحن", "Courier Inventory" } },
