    ...
    try
    {
        ...
        if (context.Database.IsRelational())
        {
            ...
            try
            {
                ...
                foreach (var table in tablesWithBranch)
                {
                    try
                    {
                        var rows = await context.Database.ExecuteSqlRawAsync($"UPDATE {table} SET BranchId = 1 WHERE BranchId IS NULL;");
                        if (rows > 0)
                        {
                            Log.Information("Backfilled BranchId = 1 in {Table} for {Count} rows.", table, rows);
                        }
                    }
                    catch (Exception tableEx)
                    {
                        Log.Warning("Failed to backfill BranchId for table {Table}: {Message}", table, tableEx.Message);
                    }
                }

                // 2. Backfill WarehouseId = 1 for tables that have a nullable WarehouseId
                string[] tablesWithWarehouse = new[]
                {
                    "Orders",
                    "PurchaseInvoices",
                    "PurchaseReturns",
                ...
                foreach (var table in tablesWithWarehouse)
                {
                    try
                    {
                        var rows = await context.Database.ExecuteSqlRawAsync($"UPDATE {table} SET WarehouseId = 1 WHERE WarehouseId IS NULL;");
                        if (rows > 0)
                        {
                            Log.Information("Backfilled WarehouseId = 1 in {Table} for {Count} rows.", table, rows);
                        }
                    }
                    catch (Exception tableEx)
                    {
                        Log.Warning("Failed to backfill WarehouseId for table {Table}: {Message}", table, tableEx.Message);
                    }
                }
            }
            catch (Exception ex)
