    ...
    public async Task<PaginatedResult<ProductSummaryDto>> GetProductsAsync(ProductFilterDto filter)
    {
        ...
        var items = await query
            ...
            .Select(x => new ProductSummaryDto(
                ...
                (x.p.Variants ?? new List<ProductVariant>()).Select(v => new ProductVariantDto(v.Id, v.Size, v.Color, v.ColorAr, v.StockQuantity, v.ReorderLevel, v.PriceAdjustment ?? 0, v.ImageUrl, v.ImagePublicId)).ToList(),
                x.p.HasTax,
                x.p.VatRate,
                x.p.CostPrice,
                x.p.CreatedAt,
