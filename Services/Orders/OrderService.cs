...
public class OrderService : IOrderService
{
    ...
    public async Task SendOrderNotificationsAsync(int orderId)
    {
        ...
        /*
        // 2. Admin Email Alert (Commented out to save email quota)
        try 
        {
            var storeSettingsForEmail = await db.StoreInfo.AsNoTracking().FirstOrDefaultAsync(s => s.StoreConfigId == 1);
            var configEmails = config["Backup:Email:To"] ?? storeSettingsForEmail?.StoreEmailAddr ?? "";
            var adminEmails = configEmails
